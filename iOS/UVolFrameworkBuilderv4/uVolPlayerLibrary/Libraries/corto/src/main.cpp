...
int main(int argc, char *argv[]) {
	...
	crt::ColorAttr *color = static_cast<crt::ColorAttr *>(encoder.data["color"]);
	if(color) {
		cout << "Color bpv; " << 8.0f*color->size/nvert << endl;
		cout << "Color q: " << color->qc[0] << " " << color->qc[1] << " " << color->qc[2] << " " << color->qc[3] << endl;
	}

	crt::GenericAttr<int> *uv = static_cast<crt::GenericAttr<int> *>(encoder.data["uv"]);
	if(uv) {
	...
	crt::GenericAttr<int> *radius = static_cast<crt::GenericAttr<int> *>(encoder.data["radius"]);
	if(radius) {
